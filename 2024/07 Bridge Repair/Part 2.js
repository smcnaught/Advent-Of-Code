...
function getAllCombinations(nums) {
  ...
  for (let i = 0; i < nums.length - 1; i++) {
    ...
    for (let leftExp of leftCombos) {
      for (let rightExp of rightCombos) {
        results.push(...combine(leftExp, rightExp));
      }
    }
  }
  ...
  for (let k = 0; k < combinations.length; k++) {
    ...
    for (let i = 0; i < comboArr.length; i++) {
      ...
      if (!isNaN(comboArr[i])) {
        ...
        else if (operator === '*') res *= +comboArr[i];
        else res = +(res.toString() + comboArr[i]);
      }
...
console.log(`Part Two Answer: ${totalCalibrationResult}`);