...
let passesAllTests = (password) => {
    let passes = newPassContainsStraight(password) && hasOnlyGoodLetters(password) && hasTwoPairs(password);
    if (passes) {
        if (!passedOnce) {
            passedOnce = true;
            return false;
...
console.log(`Part Two Answer: ${password.join('')}`)