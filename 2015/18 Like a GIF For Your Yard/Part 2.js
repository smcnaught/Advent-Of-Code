...
let mappedByPositions = grid.map(el => el.position);
let lightsThatStayOn = [(maxY * i), (maxX + (maxY * i)), i, (i + maxX)];
...
function loopXtimes(x) {
  for (let i = 0; i < x; i++) {
    grid.forEach(g => {
      if (!lightsThatStayOn.includes(g.position)) {
        nextGridState.push({ position: g.position, status: getNewStatus(g.position, g.status) })
      }
      else nextGridState.push({ position: g.position, status: "#" });
    });
  ...
  console.log(`Part Two Answer: ${lightsOn}`);
}
