...
new formatter.Formatter(__dirname).getArrayOfStringsByLine().map((line, index) => {
  ...
  for (let i = 0; i < lineArr.length; i++) {
    ...
    if (lineArr[i] === 'S') startCoords = [index, i];
...
function locationStatus(currentLocation, newLocation, grid) {
  ...
  else if (passedElevationCheck) return 'Valid';
}

function passElevationCheck(currentVal, nextVal) {
  const currentElevation = currentVal === 'S' ? 1 : (currentVal.charCodeAt(0) - 96);
  const nextElevation = nextVal === 'S' ? 1 : nextVal === 'E' ? 26 : (nextVal.charCodeAt(0) - 96);
  return (nextElevation - currentElevation) <= 1;
}

const shortestPath = findShortestPath(startCoords, grid);
console.log(`Part One Answer: ${shortestPath.length}`);