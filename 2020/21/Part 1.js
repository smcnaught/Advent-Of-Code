...
function getAllergyFreeIngredients(assignedAllergens)
{
  let noAllergyIngCount = 0;
  const assignedAllergensArr = Array.from(assignedAllergens.values());

  data.forEach(set => {
    set.ingredients.forEach(ing => {
      const ingIsAllergyFree = !assignedAllergensArr.includes(ing);
      if (ingIsAllergyFree) noAllergyIngCount++;
    })
  })

  return noAllergyIngCount;
}

const assignedAllergens = assignAllergens();
const numberOfAllergenFreeIngredients = getAllergyFreeIngredients(assignedAllergens);
console.log(`Part One Answer: ${numberOfAllergenFreeIngredients}`);