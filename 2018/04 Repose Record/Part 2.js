...
let guardAndMinuteCount = []; // { id: 10, minutesCount: [] }
guards.forEach(guard => {
  let minArr = getSpecificMinutes(guard.guard);
  let counts = {};
  minArr.forEach((x) => counts[x] = (counts[x] || 0) + 1);
  guardAndMinuteCount.push({ id: guard.guard, minutesCount: counts });
})
...
guardAndMinuteCount.forEach(x => {
  let minutes = x.minutesCount;
  let id = x.id;

  for (let min in minutes) {
    if (minutes[min] > answer.numberOfRepeats) {
      answer.guard = id;
      answer.numberOfRepeats = minutes[min];
      answer.minuteRepeated = min;
    }
  }
})

let partTwoAnswer = answer.guard * parseInt(answer.minuteRepeated);
